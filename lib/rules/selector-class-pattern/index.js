...
const messages = ruleMessages(ruleName, {
  expected: (selectorValue, messageTemplate = "Expected class selector \".${value}\" to match specified pattern") =>
    messageTemplate.replace(/\$\{value\}/g, selectorValue)
});
    ...
    function checkSelector(fullSelector, rule) {
      fullSelector.walkClasses(classNode => {
        ...
        report({
          result,
          ruleName,
          message: messages.expected(value, messageTemplate),
          node: rule,
          index: sourceIndex
        });
      });
